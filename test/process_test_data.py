...
from minigene_design._generate_sequences import generate_sequences
from minigene_design._utils import extract_exon_info
...
def process_test_data(hg38_folder: Path, input_folder: Path):
    ...
    fasta_file = hg38_folder / "GRCh38.d1.vd1.fa"
    gtf_file = hg38_folder / "gencode.v33.primary_assembly.annotation.gtf"
    ...
    merged = without_refvar(ref_df).merge(without_refvar(var_df), on="minigene_id", suffixes=("", "_drop"))
    merged = merged.rename({"minigene": "minigene_ref", "minigene_drop": "minigene_var"}, axis=1)
    dupes = [col for col in merged.columns if col.endswith("_drop")]
    merged = merged.drop(dupes, axis=1)

    return {"ref": ref_df, "var": var_df, "both": merged}
